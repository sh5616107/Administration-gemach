...
export default function BorrowerForm({ borrower, onSaved }: BorrowerFormProps) {
  ...
  const handleSave = async () => {
    ...
    const phoneCheck = await checkDuplicatePhone(formData.phone, borrower?.id)
    ...
    if (formData.id_number) {
      ...
      const duplicateId = allBorrowers.find(b =>
        b.id_number === formData.id_number && b.id !== borrower?.id
      )
      if (duplicateId) {
  ...
  const doSave = async () => {
    try {
      let savedBorrowerId: string;
      if (borrower?.id) {
        await borrowersService.update(borrower.id, formData)
        savedBorrowerId = borrower.id;
        setSnackbar({ open: true, message: 'הלווה עודכן בהצלחה', severity: 'success' })
      } else {
        const result = await borrowersService.create(formData)
        savedBorrowerId = result.lastInsertRowid as string;
        setSnackbar({ open: true, message: 'הלווה נוסף בהצלחה', severity: 'success' })
      }
      if (onSaved) onSaved(savedBorrowerId)
      setFormData(emptyBorrower)
    } catch (error) {
  ...
  return (
    <Box>
      <Typography variant="h6" sx={{ mb: 3 }}>
        {borrower ? 'עריכת לווה' : 'הוספת לווה חדש'}
      </Typography>

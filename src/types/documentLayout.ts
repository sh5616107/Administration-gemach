...
export interface DocumentLayoutConfig {
  ...
  dividerColor?: string
  /**
   * צבע ראשי — מוזרק כמשתנה CSS גלובלי (--doc-accent) דרך applyDocumentBranding,
   * ולכן חל אוטומטית על כל מסמך שה-HTML שלו מפנה ל-var(--doc-accent, ...)
   * בהגדרות הצבע שלו. נכון להיום:
   * - דוח לווה ודוח מפקיד: צבע מלא — כותרת, כותרות משנה, וקווי טבלה עליונים.
   * - שטר הלוואה / קבלת תרומה / שטר הפקדה: רק כותרת המסמך הראשית (h1) כרגע
   *   (וב-שטר הפקדה גם תג "הפקדה מחזורית"). שאר האלמנטים שם עדיין צובעים
   *   inline בקוד קשיח (ירוק/סגול/אדום תמאטיים, לא כחול-ראשי) ולא יגיבו
   *   לשדה הזה — זה בכוונה, לא באג.
   */
  accentColor?: string
  /**
   * מצב שחור-לבן — מנטרל בבת אחת את כל הצבעים במסמך (ר' הסבר מלא ב-
   * buildStyleOverrides), כולל צבעים סמנטיים שאין להם שדה ייעודי משלהם
   * (למשל הרקע הירוק על שורות משיכה בטבלת הפקדות, סטטוס "פעיל"/"נמשך"
   * בדוח לווה/מפקיד, וכו'). גובר חזותית על accentColor/dividerColor
   * (הם עדיין שמורים בקונפיג, רק לא נראים כל עוד המצב הזה פעיל).
   */
